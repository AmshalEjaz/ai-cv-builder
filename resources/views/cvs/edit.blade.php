    ...
    <form method="POST" action="{{ route('cvs.update', $cv) }}" class="panel form-stack" enctype="multipart/form-data">
        ...
        <label>Skills <small class="field-help">Separate skills with commas</small>
            <input type="text" name="data[skills]" 
                value="{{ old('data.skills', is_array(data_get($data, 'skills')) ? implode(', ', data_get($data, 'skills')) : data_get($data, 'skills', '')) }}">
        </label>
        ...
        <hr>
        <label>Replace CV file <small class="field-help">Optional — upload a new/updated PDF or DOCX and AI will re-parse it.</small>
